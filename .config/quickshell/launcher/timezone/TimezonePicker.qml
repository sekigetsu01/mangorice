                            ...
                            TextInput {
                                ...
                                Keys.onPressed: function(ev) {
                                    if ((ev.key === Qt.Key_J || ev.key === Qt.Key_Down) && (ev.modifiers & Qt.ControlModifier)) {
                                        if (root.selectedIndex < tzModel.count - 1) { root.selectedIndex++; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                                        ev.accepted = true;
                                    } else if ((ev.key === Qt.Key_K || ev.key === Qt.Key_Up) && (ev.modifiers & Qt.ControlModifier)) {
                                        if (root.selectedIndex > 0) { root.selectedIndex--; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                                        ev.accepted = true;
