...
Item {
    ...
    // ── live clock via UTC offset ─────────────────────────────────────────
    // offsetSeconds is the zone's UTC offset (already parsed from Python output).
    // We just do: utcMs + offsetSeconds*1000, then extract HH:MM.
    ...
    // Tick every 10s to keep minutes fresh
    Timer {
        id: clockTick
        interval: 10000
        repeat: true
    ...
    // ── apply timezone via sudo -S (no terminal) ──────────────────────────
    Process {
        id: applyProc
        ...
        ColumnLayout {
                            ...
                            TextInput {
                                ...
                                Keys.onPressed: function(ev) {
                                    if ((ev.key === Qt.Key_J || ev.key === Qt.Key_Down) && (ev.modifiers & Qt.ControlModifier)) {
                                        if (root.selectedIndex < tzModel.count - 1) { root.focusSource = "keyboard"; root.selectedIndex++; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                                        ev.accepted = true;
                                    } else if ((ev.key === Qt.Key_K || ev.key === Qt.Key_Up) && (ev.modifiers & Qt.ControlModifier)) {
                                        if (root.selectedIndex > 0) { root.focusSource = "keyboard"; root.selectedIndex--; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                                        ev.accepted = true;
            ...
            ListView {
                ...
                MouseArea {
                    ...
                    onWheel: function(wheel) {
                        if (wheel.angleDelta.y > 0) {
                            if (root.selectedIndex > 0) { root.focusSource = "mouse"; root.selectedIndex--; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                        } else {
                            if (root.selectedIndex < tzModel.count - 1) { root.focusSource = "mouse"; root.selectedIndex++; listView.positionViewAtIndex(root.selectedIndex, ListView.Contain); }
                        }
                        wheel.accepted = true;
                    }
                }
                ...
                delegate: Rectangle {
                    ...
                    // Recompute time whenever clockEpoch bumps
                    ...
                    RowLayout {
                        ...
                        // Fuzzy-highlighted display name
                        Item {
                            Layout.fillWidth: true
                            Layout.fillHeight: true
                            Row {
                                ...
                                Repeater {
                                    ...
                                    Text {
                                        ...
                                        color: isMatch ? theme.clrMatch
                                             : (row.isSel ? theme.clrTextPrim : theme.clrTextSecond)
                                        Behavior on color { ColorAnimation { duration: 80 } }
                                    }
                                }
                            }
                        ...
                        Item {
                            ...
                            Text {
                                ...
                                color: row.starred          ? theme.clrStar
                                     ...
                                     : (row.mouseOver || row.isSel) ? theme.clrStarOff
                                     : "transparent"
